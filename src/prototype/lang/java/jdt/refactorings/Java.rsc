...
data Id = method(list[Entity] genericTypes, str name, list[Entity] params, Entity returnType)
		...
		| typeArgument(str name, tuple[str,str] termc, Entity init) // term context
		
		| upper(Entity init)
		| lower(Entity init)
		| captured(Entity \type) // captured non-wildcard types; may have wildcard types as type arguments though;
		
		| bottom() // unknown bottom type value, subtype of all the types
        ;
